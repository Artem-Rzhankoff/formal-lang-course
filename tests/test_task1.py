...
from project.task1 import GraphSummary, get_graph_summary, create_two_cycles_graph
import pytest
...
@pytest.mark.parametrize(
    "expected_graph, cycle_node_count1, cycle_node_count2",
    [(expected_graph_1_1, 1, 1), (expected_graph_1_2, 1, 2)],
)
def test_create_two_cycles_graph(
    tmp_path, expected_graph, cycle_node_count1, cycle_node_count2
):
    path_to_actual_save = tmp_path / "graph.dot"
    create_two_cycles_graph(
        cycle_node_count1, cycle_node_count2, ("x", "y"), path_to_actual_save
    )
